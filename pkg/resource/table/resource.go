...
func (r *resource) SetIdentifiers(identifier *ackv1alpha1.AWSIdentifiers) error {
	...
	f0, f0ok := identifier.AdditionalKeys["databaseName"]
	if f0ok {
		r.ko.Spec.DatabaseName = aws.String(f0)
	}

	f1, f1ok := identifier.AdditionalKeys["catalogID"]
	if f1ok {
		r.ko.Spec.CatalogID = aws.String(f1)
	}
...
func (r *resource) PopulateResourceFromAnnotation(fields map[string]string) error {
	...
	databaseName, ok := fields["databaseName"]
	if !ok {
		return ackerrors.NewTerminalError(fmt.Errorf("required field missing: databaseName"))
	}
	r.ko.Spec.DatabaseName = &databaseName

	f1, f1ok := fields["catalogID"]
	if f1ok {
		r.ko.Spec.CatalogID = aws.String(f1)
	}
